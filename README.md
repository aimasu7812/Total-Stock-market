...
最新値タブの上段では、現在PER、日経平均EPS（`日経平均 ÷ PER`）、PER20倍から30倍までの日経平均水準、PBR、空売り比率合計、投資主体別売買動向の主要3項目、VIX指数、信用損益率、信用売り残、信用買い残、日経平均、TOPIX、NT倍率を優先表示します。EPSはnikkei225jpの現行PERページと同じ指数ベースPERから算出し、参考として加重平均EPSも併記します。空売り比率合計が40%を超えた場合は緑色、VIXが20超で黄色・30超で赤色、信用損益率が10%超で赤色、売り残が8,000億円超で青色、買い残が4兆円超で黄色・5兆円超で赤色に強調します。投資主体別売買動向はプラスを緑、マイナスを赤で表示します。
